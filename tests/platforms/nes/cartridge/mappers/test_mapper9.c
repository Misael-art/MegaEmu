/**
 * @file test_mapper9.c
 * @brief Testes para o Mapper 9 (MMC2/PxROM) do NES
 */

#include "unity.h"
#include "platforms/nes/cartridge/nes_cartridge.h"
#include "platforms/nes/cartridge/mappers/mapper9.h"
#include <stdlib.h>
#include <string.h>
...
void test_mapper9_init(void)
{
    // Testar inicialização do mapper
    nes_mapper_t *mapper = nes_mapper_9_init(ctx.cartridge);
    TEST_ASSERT_NOT_NULL(mapper);
    TEST_ASSERT_EQUAL_INT(9, mapper->number);
...
void test_mapper9_cpu_read(void)
{
    // Inicializar mapper
    nes_mapper_t *mapper = nes_mapper_9_init(ctx.cartridge);
    TEST_ASSERT_NOT_NULL(mapper);
    ...
    // O primeiro banco (0x8000-0x9FFF) deve ser selecionável
    ...
    // Verificar leitura do banco 1 (0x8000-0x9FFF)
    value = mapper->cpu_read(mapper->context, 0x8000);
    TEST_ASSERT_EQUAL_UINT8(ctx.prg_rom[0x8000], value); // Primeiro byte do banco 1

    // Último banco fixo (0xE000-0xFFFF)
...
void test_mapper9_cpu_write(void)
{
    // Inicializar mapper
    nes_mapper_t *mapper = nes_mapper_9_init(ctx.cartridge);
    TEST_ASSERT_NOT_NULL(mapper);
...
void test_mapper9_ppu_read(void)
{
    // Inicializar mapper
    nes_mapper_t *mapper = nes_mapper_9_init(ctx.cartridge);
    TEST_ASSERT_NOT_NULL(mapper);
...
void test_mapper9_ppu_write(void)
{
    // Inicializar mapper
    nes_mapper_t *mapper = nes_mapper_9_init(ctx.cartridge);
    TEST_ASSERT_NOT_NULL(mapper);
...
void test_mapper9_mirror_control(void)
{
    // Inicializar mapper
    nes_mapper_t *mapper = nes_mapper_9_init(ctx.cartridge);
    TEST_ASSERT_NOT_NULL(mapper);
...
int main(void)
{
    ...
    RUN_TEST(test_mapper9_init);
    RUN_TEST(test_mapper9_cpu_read);
    RUN_TEST(test_mapper9_cpu_write);
    RUN_TEST(test_mapper9_ppu_read);
    RUN_TEST(test_mapper9_ppu_write);
    RUN_TEST(test_mapper9_mirror_control);
    RUN_TEST(test_mapper9_reset);

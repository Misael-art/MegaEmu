...
typedef enum {
  ...
  BP_TYPE_FRAME,      // Parar em um frame específico
  BP_TYPE_SCANLINE,   // Parar em uma scanline específica
  BP_TYPE_SPRITE,     // Parar quando um sprite específico é renderizado
  BP_TYPE_DMA,        // Parar em operações DMA
  BP_TYPE_STACK,      // Parar em operações de pilha
  BP_TYPE_IO_ACCESS   // Parar em acessos a portas de I/O
} breakpoint_type_t;
...
typedef enum {
  ...
  BP_FLAG_EXPORT = (1 << 8),    // Pode ser exportado/salvo
  BP_FLAG_PROFILE = (1 << 9),   // Coleta dados de profiling
  BP_FLAG_ADVANCED = (1 << 10), // Usa recursos avançados
  BP_FLAG_GROUP = (1 << 11)     // Parte de um grupo de breakpoints
} breakpoint_flags_t;
...
typedef struct {
  ...
  // Novos campos
  uint32_t group_id;     // ID do grupo (se BP_FLAG_GROUP)
  uint64_t timestamp;    // Timestamp da última ativação
  uint32_t profile_hits; // Contador para profiling
  float avg_cycles;      // Média de ciclos entre hits
  void *custom_data;     // Dados específicos do tipo
  uint32_t custom_size;  // Tamanho dos dados específicos

  // Callbacks personalizados
  void (*on_hit)(struct breakpoint_t *bp, void *context);
  void (*on_condition)(struct breakpoint_t *bp, void *context);
  void (*on_remove)(struct breakpoint_t *bp, void *context);
} breakpoint_t;
...
typedef void (*breakpoint_callback_t)(breakpoint_context_t *context,
 ...
 * @return breakpoint_context_t* Ponteiro para o contexto ou NULL em caso de
 * erro
 */
breakpoint_context_t *breakpoint_create_context(uint32_t platform_id,
 ...
 */
bool breakpoint_set_callback(breakpoint_context_t *context,
                             breakpoint_callback_t callback, void *user_data);
 ...
 */
int32_t breakpoint_add(breakpoint_context_t *context, breakpoint_type_t type,
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_add_conditional(breakpoint_context_t *context,
                                   breakpoint_type_t type, uint32_t address,
                                   breakpoint_compare_t cmp, uint32_t value,
                                   uint32_t flags);
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_add_with_expression(breakpoint_context_t *context,
                                       breakpoint_type_t type, uint32_t address,
                                       const char *condition, uint32_t flags);
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_add_range(breakpoint_context_t *context,
                             breakpoint_type_t type, uint32_t address_start,
                             uint32_t address_end, uint32_t flags);
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_remove(breakpoint_context_t *context, int32_t id);
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_enable(breakpoint_context_t *context, int32_t id, bool enable);
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_modify(breakpoint_context_t *context, int32_t id,
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_get_info(breakpoint_context_t *context, int32_t id,
                         breakpoint_t *bp);
 ...
 */
uint32_t breakpoint_list(breakpoint_context_t *context, breakpoint_t *bps,
                         uint32_t max_bps);
 ...
 * @param id_out Ponteiro para receber o ID do breakpoint ativado (pode ser
 * NULL)
 * @return bool true se algum breakpoint foi ativado, false caso contrário
 */
bool breakpoint_check_execution(breakpoint_context_t *context, uint32_t pc,
                                const void *regs, int32_t *id_out);
 ...
 * @return bool true se algum breakpoint foi ativado, false caso contrário
 */
bool breakpoint_check_memory(breakpoint_context_t *context, uint32_t address,
                             uint32_t value, bool is_write, int32_t *id_out);
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_set_temporary(breakpoint_context_t *context,
                                 uint32_t address);
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_set_description(breakpoint_context_t *context, int32_t id,
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_set_log_format(breakpoint_context_t *context, int32_t id,
                               const char *format);
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_set_hit_count(breakpoint_context_t *context, int32_t id,
                              uint32_t count);
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_add_register(breakpoint_context_t *context,
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_add_watchpoint(breakpoint_context_t *context,
                                  uint32_t address, breakpoint_compare_t cmp,
 ...
 * @return int32_t ID do breakpoint ou valor negativo em caso de erro
 */
int32_t breakpoint_add_cycle_count(breakpoint_context_t *context,
                                   uint32_t cycles, uint32_t flags);
 ...
 * @return bool true se bem-sucedido, false caso contrário
 */
bool breakpoint_export(breakpoint_context_t *context, const char *filename);
 ...
 * @param context Ponteiro para o contexto
 * @param filename Nome do arquivo
 * @return int32_t Número de breakpoints importados ou valor negativo em caso de
 * erro
 */
int32_t breakpoint_import(breakpoint_context_t *context, const char *filename);

...
void sms_psg_end_frame(sms_psg_t *psg) {
  ...
  SMS_PSG_LOG_TRACE("Frame de áudio finalizado, amostras geradas: %d",
                    psg->buffer_pos);
}
 ...
 */
uint32_t sms_psg_get_samples(sms_psg_t *psg, int16_t *buffer,
  ...
  uint32_t samples_to_copy =
      (psg->buffer_pos < num_samples) ? psg->buffer_pos : num_samples;
  ...
  if (value & 0x80) {
    ...
    if (value & 0x10) {
      ...
      case 1: // Tone 0 frequency (high)
        psg->tone_periods[0] =
            (psg->tone_periods[0] & 0x00F) | ((data & 0x3F) << 4);
        break;
      ...
      case 3: // Tone 1 frequency (high)
        psg->tone_periods[1] =
            (psg->tone_periods[1] & 0x00F) | ((data & 0x3F) << 4);
        break;
      ...
      case 5: // Tone 2 frequency (high)
        psg->tone_periods[2] =
            (psg->tone_periods[2] & 0x00F) | ((data & 0x3F) << 4);
        break;
...
int sms_psg_register_save_state(sms_psg_t *psg, save_state_t *state) {
  ...
  // Implementação simplificada - em uma implementação real, registraria todos
  // os campos necessários
  save_state_register_section(state, "SMS_PSG");

  save_state_register_field(state, psg->tone_periods,
                            sizeof(psg->tone_periods));
  save_state_register_field(state, &psg->noise_period,
                            sizeof(psg->noise_period));
  save_state_register_field(state, &psg->mixer, sizeof(psg->mixer));
  save_state_register_field(state, psg->volumes, sizeof(psg->volumes));
  save_state_register_field(state, &psg->envelope_period,
                            sizeof(psg->envelope_period));
  save_state_register_field(state, &psg->envelope_shape,
                            sizeof(psg->envelope_shape));
  save_state_register_field(state, psg->tone_counters,
                            sizeof(psg->tone_counters));
  save_state_register_field(state, psg->tone_states, sizeof(psg->tone_states));
  save_state_register_field(state, &psg->noise_counter,
                            sizeof(psg->noise_counter));
  save_state_register_field(state, &psg->noise_shift, sizeof(psg->noise_shift));
  save_state_register_field(state, &psg->noise_output,
                            sizeof(psg->noise_output));
  save_state_register_field(state, &psg->envelope_counter,
                            sizeof(psg->envelope_counter));
  save_state_register_field(state, &psg->envelope_step,
                            sizeof(psg->envelope_step));
  save_state_register_field(state, &psg->latch_register,
                            sizeof(psg->latch_register));

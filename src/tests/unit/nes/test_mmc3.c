/**
 * @file test_mmc3.c
 * @brief Testes unitários para o mapper MMC3
 */
...
static mmc3_test_context_t test_ctx = {0}; // Inicializar com zero

void setUp(void)
{
    // Limpar o contexto
    memset(&test_ctx, 0, sizeof(mmc3_test_context_t));
    ...
    test_ctx.cartridge->mapper_number = 4; // MMC3
...
{
    TEST_ASSERT_NOT_NULL(test_ctx.cartridge);
    TEST_ASSERT_EQUAL_INT(4, test_ctx.cartridge->mapper_number);
    TEST_ASSERT_NOT_NULL(test_ctx.cartridge->mapper);
...
void test_mmc3_bank_switching(void)
{
    ...
    nes_cartridge_cpu_write(test_ctx.cartridge, 0x8000, 0x00); // Selecionar registro
    nes_cartridge_cpu_write(test_ctx.cartridge, 0x8001, 0x01); // Selecionar banco 1
    ...
    nes_cartridge_cpu_write(test_ctx.cartridge, 0x8000, 0x02); // Selecionar registro
    nes_cartridge_cpu_write(test_ctx.cartridge, 0x8001, 0x01); // Selecionar banco 1
